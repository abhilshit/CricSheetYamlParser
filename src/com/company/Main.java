...
public class Main {
    ...
    private static final String[] quarters = {"Q1", "Q1", "Q1", "Q2", "Q2", "Q2", "Q3", "Q3", "Q3", "Q4", "Q4", "Q4"};

    private static final String[] batsmen = {"Kohli"};
    private static StringBuffer csv_data = new StringBuffer();
    public static void main(String[] args) throws IOException{

        csv_data.append("batsman, "+"opposite_team" + ", " + "location"+ ", "+ "date"+", "+ "quarter_of_year"+", "+"chasing"+", "+"batsmanScoreIn10Overs"+", "+"indiaScoreIn10Overs"+", "+"batsmanScoreIn20Overs"+", "+"indiaScoreIn20Overs"+", "+"batsmanScoreIn30Overs"+", "+"indiaScoreIn30Overs"+", "+"batsmanScoreIn40Overs"+", "+"indiaScoreIn40Overs"+", "+"batsmanScoreIn50Overs"+", "+"indiaScoreIn50Overs"+", "+"result"+"\n");

        for(String batsman:batsmen) {

            Files.newDirectoryStream(Paths.get("./yaml"),
                    path -> path.toString().endsWith(".yaml"))
                    .forEach(filePath -> convertFile(filePath,batsman));
    ...
    private static void convertFile(Path path,String batsmanName) {
        String yamlContent = null;
        try {
            yamlContent =  new String(Files.readAllBytes(path));
            Yaml yaml= new Yaml();
            Map<String,Object> map= (Map<String, Object>) yaml.load(yamlContent);
            Map<String,Object> matchInfo =  ((Map)map.get("info"));
            ArrayList<String> teams = (ArrayList<String>)matchInfo.get("teams");
            String firstTeam = teams.get(0);
            String secondTeam = teams.get(1);
            String opposite_team = ("india").equalsIgnoreCase(firstTeam)?secondTeam:firstTeam;

            String winning_team = (String)((Map)matchInfo.get("outcome")).get("winner");
            String result = ("india").equalsIgnoreCase(winning_team)?"INDIA_WON":"INDIA_LOST";
            String location = (String)matchInfo.get("venue");
            location = location.replace(",", "");
            location = NormalizationUtil.normalizedStadiumMap.get(location);
            Date matchDate = (Date)(((ArrayList<Date>)matchInfo.get("dates")).get(0));
            Calendar calendar = Calendar.getInstance();
            calendar.setTime(matchDate);
            int month = calendar.get(Calendar.MONTH);
            String quarter_of_year = quarters[month];

            Map firstInningsMap =  (Map)((Map)((ArrayList) map.get("innings")).get(0)).get("1st innings");
            String firstInningsTeam = (String)firstInningsMap.get("team");

            ArrayList firstInningDeliveries =  (ArrayList)firstInningsMap.get("deliveries");

            Map secondInningsMap =  (Map)((Map)((ArrayList) map.get("innings")).get(1)).get("2nd innings");
            String secondInningsTeam = (String)secondInningsMap.get("team");

            ArrayList secondInningDeliveries =  (ArrayList)secondInningsMap.get("deliveries");

            boolean chasing = ("india").equalsIgnoreCase(firstInningsTeam)?false:true;
            ...
            if(batsmanScoreIn10Overs==0 && batsmanScoreIn20Overs==0 && batsmanScoreIn30Overs==0 && batsmanScoreIn40Overs==0 && batsmanScoreIn50Overs==0){
                batsmanPlayed = false;
            }
            if(batsmanPlayed) {
                csv_data.append(batsmanName+", "+opposite_team + ", " + location + ", " +matchDate.toString()+", "+ quarter_of_year + ", " + chasing + ", " + batsmanScoreIn10Overs + ", " + indiaScoreIn10Overs + ", " + batsmanScoreIn20Overs + ", " + indiaScoreIn20Overs + ", " + batsmanScoreIn30Overs + ", " + indiaScoreIn30Overs + ", " + batsmanScoreIn40Overs + ", " + indiaScoreIn40Overs + ", " + batsmanScoreIn50Overs + ", " + indiaScoreIn50Overs + ", " + result + "\n");
            }
